...
export class DitheringEffect extends Effect {
  ...
  /**
   * Enables or disables dithering effect
   * @param enabled - Whether the dithering effect is enabled
   */
  setDitheringEnabled(enabled) {
    const ditheringEnabledUniform = this.uniforms.get("ditheringEnabled")
    if (ditheringEnabledUniform !== undefined) {
      ditheringEnabledUniform.value = enabled ? 1 : 0
    }
  }
}
